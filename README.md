...
Åpne deretter `powerbi/VV_Styringsdashboard.pbip`. Rapporten er ferdig bygget og ligger i repoet som TMDL og JSON — sider, mål, beregningsgruppe og radnivåsikkerhet følger med. Den eneste tilpasningen som kan trengs er stiparameteren i Power Query, dersom prosjektet ligger et annet sted enn `C:\prosjekt\vv-ventetid\`.

Krever Power BI Desktop med `.pbip`-format og TMDL aktivert under Preview features.

`powerbi/STEG_FOR_STEG.md` er ikke nødvendig for å bruke rapporten. Den er en logg over hvordan den ble bygget og hvorfor hvert valg ble tatt. `START_HER.md` tar installasjon og kjøring steg for steg, inkludert den manuelle NKI-eksporten.
